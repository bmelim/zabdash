...
//print_r($disk->hosts[0]->hostid);

    ////$diskSize = get_item_values($disk->itemid, 'sysMemorySizeinbytes');
    
    //$diskUsed = get_item_values($disk->itemid, 'sysMemoryUsedinbytes');	  
    
 //  $diskSize = get_item_values($disk->itemid, 'ifInOctets');
 //  $diskUsed = get_item_values($disk->itemid, 'ifOutOctets');
...
/*
	...
	if($diskSize['value_max'] != 0 ) {			
		...
		//$arrDesc[]= get_item_label($diskSize['key_']);
	}
	...
	//}
		...
		if($diskSize['key_'] != '' ) {			
			$arrDesc[]= get_item_label($diskSize['key_']).",".$diskSize['value_max'];
		//$arrDesc[]= get_item_label($diskSize['key_']);
		}
*/		
	
/*	
	$ifDesc[] = get_item_values($disk->itemid, 'ifDesc');
 
	if($ifDesc['key_'] != '' ) {			
		$arrDesc[]= get_item_label($ifDesc['key_']);
	}
	*/			
 }
